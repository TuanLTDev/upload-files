...
export class AbstractInputValidatorInterceptor {
     ...
     */
    getSchema(req) {
        throw new Error(`${AbstractInputValidatorInterceptor.name}  need to implement getSchema`);
    }
    ...
    getValueToValidate(req) {
        throw new Error(`${AbstractInputValidatorInterceptor.name} need to implement getValueToValidate`);
    }

    /**
     * @returns error handler of joi. Base handler is the utility function responseJoiError
     */
    getResponseErrorHandler(res, error) {
