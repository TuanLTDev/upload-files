export class SwaggerDocument {
    ...
    static ApiProperty(options) {
        const { type, model = SwaggerDocument.DEFAULT_GENERATOR, required = true, readOnly = false, example } = options;
        let swaggerType;
    ...
    static ApiParams(options) {
        const { type, name, model, paramsIn = 'query', required = true, example, description } = options;
